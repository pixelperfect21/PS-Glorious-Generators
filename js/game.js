...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		return gain.floor().sub(player[layer].points).add(1).min(player[layer].best.sub(player[layer].points)).max(1);
	} else if (type=="normal"){
...
function rowReset(row, layer) {
	for (lr in ROW_LAYERS[row]){
		if(layers[lr].doReset) {
			if (!isNaN(row+1)) Vue.set(player[lr], "activeChallenge", null) // Exit challenges on any row reset on an equal or higher row
